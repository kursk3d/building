...
export class ImspartanGDOne{
    ...
    private jData:ImspartanProviderGDOne;
    private screenData:ImspartanScreenDataGDOne;
    private platform:ImspartanPlatformGDOne;
    public slider:ImspartanSliderGDOne;

    private resizeInit:boolean = true;
...
// Прослушка события изменения ориентации
        mql.addListener((m)=> {
            if(m.matches) {console.log('// Изменено на портретный режим');}
            else {console.log('// Изменено на горизонтальный режим');}
            this.resizePlatform();
        });
