...
export class ImspartanSliderScreenGDOne{
    // Параметры самого слайдера
    wBase:number;
    hBase:number;
    wSlider:number;
    hSlider:number;
    name:string = 'def-name_slider';
    sliderMarginStr:string;

    stage:{class:string,style:string} = {class:'',style:''};

    // карточки для слайдера
    card:ImspartanSliderScreenCard;

    private static instance: ImspartanSliderScreenGDOne;
    private constructor(){
